...
| 폴더 | 무엇 | 학습 핵심 | 상태 |
|---|---|---|---|
| [sonlm-from-scratch](sonlm-from-scratch/) | **Decoder-only** transformer (GPT 계열) | causal self-attn, autoregressive 생성, x/y shift | ✅ 정답지 + 가이드 |
| [encoder-decoder-from-scratch](encoder-decoder-from-scratch/) | **Encoder-Decoder** (원조 2017 paper) | **cross-attention**, sinusoidal PE, teacher forcing | ✅ 정답지 + 가이드 |
| [bert-from-scratch](bert-from-scratch/) | **Encoder-only** (BERT) + MLM | 양방향 attention, 15% 마스킹, [CLS] | ✅ 정답지 |
...
### 🟢 Tier 1 — 모던 LLM 변형 (decoder-only를 더 깊게)
...
### 🔴 Tier 4 — Post-training (Alignment)
...
| 시나리오 | 추천 순서 |
|---|---|
| **모던 LLM 풀스택 마스터** | 1. ✅ sonlm → 2. **llama** (Tier 1) → 3. **kv-cache** → 4. **lora** → 5. **dpo** |
| **Transformer 가족 다 보기** | 1. ✅ sonlm → 2. ✅ encoder-decoder → 3. ✅ bert → 4. **vit** → 5. **whisper-mini** |
| **연구자 트랙** | 1. ✅ sonlm → 2. **scaling-laws-mini** → 3. **attention-viz** → 4. 관심 분야 깊게 |
| **응용 엔지니어 트랙** | 1. ✅ sonlm → 2. **lora** → 3. **kv-cache** → 4. **speculative-decoding** |
